...
class Solution:
    def numJewelsInStones(self, J: str, S: str) -> int:
        J_s = set(list(J))
        return sum([S.count(e) for e in J_s])
    ...
    def numUniqueEmails(self, emails: List[str]) -> int:
        ...
        for email in emails:
            ...
            if match:
                ...
                # remove after '+' sign
                localname = re.sub(r'\+.*', '', localname)
                # remove '.'
                localname = localname.replace('.', '')

                result.add(localname + match.group(2))

        return len(result)

    def numSubarrayProductLessThanK_1sttry(self, nums: List[int], k: int) -> int:
        result = 0
        for l in range(1, len(nums)+1):
            for start_index in range(0, len(nums)+1-l):
                subarr = nums[start_index:start_index+l]
                ...
                ## 大きな入力に対して、遅すぎる
                # if functools.reduce(lambda x, y: x*y, subarr) < k:
                #     result += 1
        return result
...
# e1 = "(()())(())"
# print(a.removeOuterParentheses(e1))

# 709. To Lower Case
# e1 = "Hello"
# print(a.toLowerCase(e1))
